...
codexctl supervisor inbox --after-tick 10 --timeout-ms 600000 --limit 5 --json
codexctl supervisor ack default --tick 42 --json
...
`supervisor inbox` is the batch controller shortcut for multiple jobs. It returns `{ at, start, wait, cursor, ack, totalActions, hasMore, items }`, where each item is `{ action, inspection }` for a fresh wait action. Items are sorted by severity (`critical`, then `attention`, then `info`) while preserving supervisor order within the same severity. Pass `--limit <n>` to cap the number of inspected items. Pass `--cursor <name>` to read a controller-local cursor from the repo-scoped supervisor cursor directory; if `--after-tick` is also supplied, it overrides the stored cursor for that call. `ack` is present only when `hasMore` is false, so a controller cannot accidentally advance past unreturned actions from the same tick. Like `next`, inbox uses the pre-start cursor when no explicit or stored cursor is available and inspects wait-selected actions directly instead of re-planning. It does not apply actions, resolve approvals, cancel jobs, mutate job records, or execute `nextCommand`.

`supervisor ack <cursor-name> --tick <n>` writes a local controller cursor for future `supervisor inbox --cursor <cursor-name>` calls. It returns `{ at, previous, cursor }` and serializes per-cursor writes so concurrent acknowledgements never move an existing cursor backward. This is only a tick receipt: it does not mark individual actions resolved, suppress persistent action IDs, mutate jobs, or affect supervisor reconciliation.
...
Job keys are restricted to letters, numbers, `.`, `_`, and `-`. Existing job records in the active state root are not overwritten unless callers pass `--force`.

By default, persisted files live under `~/.codexctl/repos/<repo-id>/`, where `<repo-id>` is derived from the command's working directory. Set `CODEXCTL_HOME` to change the home directory, or `CODEXCTL_STATE_DIR` to use an explicit state root such as `.codexctl` for local dogfood runs, tests, or access to pre-home-directory records.

Job files live in `jobs/<job-key>/` under that state root:
...
Supervisor files live in `supervisor/` under that state root:
