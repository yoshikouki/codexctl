...
`supervisor inbox` is the batch controller shortcut for multiple jobs. It returns `{ at, start, wait, items }`, where each item is `{ action, inspection }` for a fresh wait action. Items are sorted by severity (`critical`, then `attention`, then `info`) while preserving supervisor order within the same severity. Pass `--limit <n>` to cap the number of inspected items. Like `next`, it uses the pre-start cursor by default and inspects wait-selected actions directly instead of re-planning. It does not apply actions, resolve approvals, cancel jobs, mutate job records, or execute `nextCommand`.
